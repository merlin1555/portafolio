...
{
	// Bootstrap Css
    wp_register_style('online-css', 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css', 'all');
    // Css Local
...
function mis_script() {
    if (!is_admin()) {
        // Bootstrap Js
        wp_enqueue_script('online-js', 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js', array('jquery'), '5.3.0', true);
        // Js Local
...
// Verificar si el usuario ha iniciado sesión y Guarda la id del usuario para la obtencion de Logros
if (is_user_logged_in()) {
    $user_id = get_current_user_id();  // Obtener el ID del usuario actual

    $usuario_id = $user_id; // Guardar el ID del usuario en una variable para usar en funciones posteriores

    mi_funcion_personalizada($usuario_id);
}
...
// Función para registrar un logro